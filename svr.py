...
from sklearn import pipeline, grid_search, linear_model
...
clf = pipeline.Pipeline([('svr', svr)])
param_grid = {'svr__C' : (1e0, 1e1, 1e2, 1e3), 'svr__gamma': np.logspace(-2, 2, 5)}
...
pd.DataFrame({"id": id_test, "relevance": y_pred}).to_csv('svr.csv',index=False)
